# 教程: 命令行界面cking Buttons to Load More Content with Crawl4AI
...
1. **Step-by-step (Session-based) 应用roach:** Multiple calls to `arun()` to progressively load more content.
2. **Single-call 应用roach:** Execute a more complex JavaScript snippet inside a single `arun()` call to handle all clicks at once before the extraction.
...
async with AsyncWebCrawler(headless=True, verbose=True) as crawler:
    ...
    # 2. 命令行界面ck the 'Next' button and wait for new content
