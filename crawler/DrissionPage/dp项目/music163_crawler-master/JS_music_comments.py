...
async def do_music_data(f,title,href,data):
    music = {
        '歌曲主题':[],
        '歌曲链接':[],
        '评论用户':[],
    ...
    if hotcomments != None:
        ...
        await f.write(json.dumps(music,ensure_ascii=False)+'\n')
...
async def fetch(s,session,music):
    # print("发送请求::",url)
    title = music[0]
    ...
    async with session.post(url,params=params,cookies=cookies,headers=headers,data=data) as response:
        async with aiofiles.open('music_data.csv','a') as file:
            music_comments_data =await response.text()
            await do_music_data(file,title,music[1],json.loads(music_comments_data))

async def main():
    muesic_list = music_href_list()
    with open('comments.js') as file:
            s = file.read() 
    async with aiohttp.ClientSession() as session:
        tasks = [asyncio.create_task(fetch(s,session,music)) for music in muesic_list]
