...
async def example_basic_usage():
    ...
    c4a_script = """# Wait for page to load
    WAIT `body` 2
...
async def example_dynamic_loading():
    ...
    pagination_script = """# Initial wait
    WAIT `.product-list` 5
...
async def example_multi_step_workflow():
    ...
    workflow_script = """# Define login procedure
    PROC login
      CLICK `#username`
      TYPE "demo_user"
      CLICK `#password`
...
async def example_combining_with_extraction():
    ...
    prep_script = """# Expand all collapsed sections
